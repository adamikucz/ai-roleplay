...
export async function createSession(input: { userId: string; characterId: string; title?: string; scene: SceneState }) {
  const result = await query<{ id: string }>(
    `insert into sessions (user_id, character_id, title, scene_state) values ($1,$2,$3,$4) returning id`,
    [input.userId, input.characterId, input.title ?? 'New Scene', JSON.stringify(input.scene)]
  );
...
export async function listSessions(userId: string) {
  const result = await query(
    `select s.id, s.title, s.character_id as "characterId", c.name as "characterName", s.scene_state as "sceneState", s.updated_at as "updatedAt"
